  ...
  void Push(const Func& func, const OpContext& ctx, bool recording,
    ...
    q_.push([=]() mutable {
      ...
      try {
        ...
        LOG(WARNING) << "in custom impl" << std::this_thread::get_id();
        // profiler::CustomOpProfiler::Get()->delayed_profile_operator_[std::this_thread::get_id()]->start(
        //   profiler::CustomOpProfiler::Get()->tid_to_dev_type_[std::this_thread::get_id()],
        //   profiler::CustomOpProfiler::Get()->tid_to_dev_id_[std::this_thread::get_id()]
        // );
        func();
        // profiler::CustomOpProfiler::Get()->delayed_profile_operator_[std::this_thread::get_id()]->stop();
