...
using TaskPtr = std::shared_ptr<profiler::ProfileTask>;

static profiler::ProfileDomain custom_op_domain("Custom Operator");
...
public:
  ...
  void OnCustomBegin(const std::string& op_type) {
    ...
    tasks_[tid] = std::make_shared<profiler::ProfileTask>(task_name.c_str(), &custom_op_domain);
    tasks_[tid]->start();
  }
  ...
  std::unordered_map<Tid, std::shared_ptr<profiler::ProfileOperator>> delayed_profile_operator_;
  std::unordered_map<Tid, mxnet::Context::DeviceType> tid_to_dev_type_;
  std::unordered_map<Tid, int32_t> tid_to_dev_id_;
